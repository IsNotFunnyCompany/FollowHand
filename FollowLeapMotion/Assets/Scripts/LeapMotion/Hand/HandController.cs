...
public class HandController : MonoBehaviour {
	...
	private LeapMaster _leapManager;
	// Use this for initialization
	void Start () {
		_leapManager = (GameObject.Find("LeapManager")as GameObject).GetComponent(typeof(LeapMaster)) as LeapMaster;
	}
	
	// Update is called once per frame
	void Update () {
		Hand primeHand = _leapManager.manoInPrimoPiano();

		if(primeHand.IsValid)
		{
			Vector3[] fingerPositions = _leapManager.getWorldFingerPosition(primeHand);
